...
function formatAsCode(obj: Obj): string {
  switch (obj.type) {
    ...
    case 'CSV': // array of arrays
      const records = parse(obj.data, { skip_empty_lines: true });
...
function parseInput(obj: Obj): any {
  switch (obj.type) {
    ...
    case 'CSV': // array of arrays
      return parse(obj.data, { skip_empty_lines: true });
...
async function replaceImports(code: string): Promise<string> {
  const imports = [...(await parseImports(code))];
  let offset = 0;
  let newCode = `${code}`;
  for (const importObj of imports) {
    const importCode = code.slice(importObj.startIndex, importObj.endIndex);
    const pkgName = importObj.moduleSpecifier.value;
    if (pkgName?.startsWith('https://')) {
      continue;
    }
    if (importObj.moduleSpecifier.type !== 'package' || !pkgName) {
...
export async function runJSScript(
  ...
  const runFunc = await replaceImports(code);
...
export async function testJSScript(
  ...
  const runFunc = await replaceImports(code);
  const testFunc = await replaceImports(testCode);
...
export function formatJSScript(
  ...
  // TODO: Use eslint?
  return format(code, { parser: 'babel' }).trim();
}
